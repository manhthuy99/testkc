...
function newGameButtonHandler() {
    ...
    setTimeout(() => {document.getElementById('timer').classList.add('time-countdown')}, 1);
    
    var count = setInterval(() => {
        timeLeft = document.getElementById('timer').offsetWidth;
        if (timeLeft >= 320) {
            document.getElementById('timer').style.backgroundColor = 'rgb(30, 252, 30)'
            console.log(timeLeft)
        } else if (timeLeft >= 160 && timeLeft < 320) {
                document.getElementById('timer').style.backgroundColor = 'rgb(254, 195, 14)'
            console.log(timeLeft)
...
function soundButtonHandler() {
    ...
    console.log(document.getElementById('timer').offsetWidth)
}
...
//        console.log(arr)
        ...
        setTimeout(autoCheck, 400);
        setTimeout(() => {console.log('Count Consecutive: ' + (countConsecutive - 1))}, 400)
...
function secondCell(e) {
    ...
    //check if there's any posible combos
    setTimeout(() => {
        if (findAllHints().length == 0 &&
        document.getElementById('bombs-cooler').textContent == 0 &&
        document.getElementById('treasure').textContent == 0 &&
...
//autocheck can't be executed one by one, need to make it delay a bit later after each check
